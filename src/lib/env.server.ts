 ...
 * `.env` (GROQ_API_KEY_1, GEMINI_API_KEY, SUPABASE_SERVICE_ROLE_KEY…)
 * look "missing" even though they were configured.
 *
 * `serverEnv()` reads `process.env` first (hosted runtime / Lovable Cloud
 * + Supabase secrets) and falls back to parsing a local `.env` file once,
 * which only succeeds in local dev where a real filesystem exists.
 */
