...
export function ScoreBar({ score, tone = "brand" }: { score: number; tone?: "brand" | "accent" | "amber" }) {
  const color = tone === "accent" ? "bg-accent" : tone === "amber" ? "bg-amber-500" : "bg-brand";
  const text = tone === "accent" ? "text-accent" : tone === "amber" ? "text-amber-500" : "text-brand";
  return (
...
export function Avatar({ initials, tone = "brand", size = "md" }: { initials: string; tone?: "brand" | "accent"; size?: "sm" | "md" | "lg" }) {
  const s = size === "sm" ? "size-8 text-[10px]" : size === "lg" ? "size-12 text-sm" : "size-10 text-xs";
  const bg = tone === "accent" ? "bg-accent/15 text-accent" : "bg-brand/15 text-brand";
  return <div className={`grid ${s} shrink-0 place-items-center rounded-full font-bold ${bg}`}>{initials}</div>;
}
...
export function StatusPill({ status }: { status: Candidate["status"] }) {
  ...
  return (
    <span className={`inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ring-1 ${map[status]}`}>
      {status}
    </span>
  );
}
...
export function StatTile({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      {delta && (
        <div className={`mt-1 text-xs font-medium ${positive ? "text-accent" : "text-destructive"}`}>
          {delta} vs last month
        </div>
      )}
    </div>
...
export function SectionCard({
  ...
  return (
    <div className={`overflow-hidden rounded-xl border border-border bg-card shadow-[0_1px_2px_rgba(0,0,0,0.02)] ${className}`}>
      {title && (
